...
Connector control commands form a separate narrow class from ordinary shell execution. Only session-local `codex-intent admit`, `status`, `amend`, and `verify` commands are recognized; admission uses inline JSON, amendment coordinates come from the issued ticket, and explicit verification can only target the current session and repository. This keeps coordination control available without granting general shell mutation authority.

Verified completion is a state transition, not a model assertion. On `Stop`, the connector collects the concrete repository delta, including untracked files, executes declared acceptance on the same worktree with tree-integrity checks, and passes the resulting manifest through the canonical integration verifier. Only a clean report transitions the active intent to `completed` and the Codex task to `verified`. Failed evidence leaves the intent available for repair and records an explicit `verification_failed` state.

The first failed Stop can request one continuation turn with deterministic findings. If that continuation reaches Stop and still fails, the connector reports `UNVERIFIED` without requesting another continuation. This preserves the completion gate while bounding model-loop behavior. Connector-owned `.codex` control files are excluded from task-change accounting; they remain protected mutation surfaces and are never grantable through the session ChangeIntent.
