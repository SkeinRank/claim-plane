...
The protocol does not infer that an ordered relationship is safe merely because an order exists. An already active worker may have built premises against the previous state. Runtime premise fencing revokes that worker's governed mutation capability as soon as its premise becomes stale, including already prepared broker operations. Runtime recovery then refuses to refresh until the stale-causing producer is completed, preserves the declared authority surface, requires a new pinned base, and records an explicit resume before a replacement broker can start. `ordered` expansion therefore remains blocked until this lifecycle establishes a fresh execution premise.
