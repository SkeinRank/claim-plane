...
The protocol does not infer that an ordered relationship is safe merely because an order exists. An already active worker may have built premises against the previous state. Runtime premise fencing can now revoke that worker's governed mutation capability as soon as its premise becomes stale, including already prepared broker operations, but refresh/rebase/resume still requires a fresh lifecycle transition. `ordered` expansion therefore remains blocked until that transition is orchestrated.
