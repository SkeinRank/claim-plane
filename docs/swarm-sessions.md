...
A ready worker result is snapshotted from its Claim Plane-owned branch into a commit. Codex control files and `.claim-plane` state are excluded. Before composition, Deterministic Integration v2 derives the actual changed paths and line regions, maps Python hunks to structural owners, and checks that the concrete mutation surface remains inside the admitted path, region, symbol, contract, or schema authority. The configured integration target branch is metadata only at this stage and is never checked out, reset, or advanced.

The snapshot is then replayed with `git cherry-pick --no-commit`. Claim Plane reconstructs the staged mutation surface after line movement, rejects staged path or semantic-owner drift, rebuilds semantic dependency evidence over the composed worktree, and compares the actual mutation roots with already integrated work. Independent or explicitly commutative changes may compose; ordered or same-resource overlap is accepted only when the worker was refreshed on the current integration head and the earlier work is an effective dependency. Unknown or conflicting actual semantics fail closed.
